...
async def get_feed_object_construction_point(request: Request) -> ObjectConstructionFeedSchema:
    api_key = request.headers.get("api-key")
    result = await UserLogicManager.get_all_object_construction_logic(api_key=api_key)
    return result
...
@router.delete("/delete_work/{obj_id}", response_model=SuccessfulResponse)
async def delete_profile_object_construction_point(obj_id: int, request: Request) -> SuccessfulResponse:
    api_key = request.headers.get("api-key")
    await UserLogicManager.delete_object_constraction_profile(api_key)
    return SuccessfulResponse(result=True)
...
def get_files_point(request: Request, file_name: str):
    file_path = UserLogicManager.get_files_logic(file_name)
